...
public class D {
	...
	class Entry {
		...
		public Entry(int wid, int ns, long sum) {
			this.wid = wid;
			this.ns = ns;
			this.sum = sum;
		}
		...
		@Override
		public int hashCode() {
			return (int) ((sum * 1272467) ^ (128 * wid + ns));
		}
		
		@Override
		public boolean equals(Object obj) {
			Entry that = (Entry) obj;
			return sum == that.sum && wid == that.wid && ns == that.ns;
		}
	}
	
	int solve(int wid, int ns, long sum) {
		if (wid == 0) {
			if (ns != n) {
				return 0;
			}
		...
		Entry entry = new Entry(wid, ns, sum);
