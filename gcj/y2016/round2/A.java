...
public class A {
	final static String GAME = "PRS";
	final static int BEAT = 1;
	final static int T = GAME.length();
	
	int n;
	int[] a;
	
	String solve() {
		String[][] game = new String[n + 1][T];
		for (int m = 0; m <= n; m++) {
			for (int i = 0; i < T; i++) {
				if (m == 0) {
					game[m][i] = "" + GAME.charAt(i);
					continue;
				}
				String gThis = game[m - 1][i];
				String gThat = game[m - 1][(i + BEAT) % T];
