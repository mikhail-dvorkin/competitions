...
public class B_old {
	...
	String solve() {
		Arrays.sort(sources);
		if (tNeed < sources[0].t || tNeed > sources[n - 1].t) {
			return "IMPOSSIBLE";
		}
		...
		double high = 1e1 * vNeed;
		while (low + 0.5e-8 < high) {
			...
			for (int q = 0; q < 2; q++) {
				...
				double vt = 0;
				for (int i = 0; i < n; i++) {
					double take = Math.min(array[i].rate * time, v);
					v -= take;
					vt += take * array[i].t;
				}
				assert Math.abs(v) < eps;
				res[q] = vt;
			}
	...
	public static void main(String[] args) throws Exception {
		...
		String fileName = B_old.class.getSimpleName().replaceFirst("_.*", "").toLowerCase() + fileNameSuffix;
